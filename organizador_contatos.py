...
def obter_imagem_gatinho_b64() -> str:
    ...
    caminho_local = os.path.join(os.path.dirname(os.path.abspath(__file__)), "gatinho.jpg")
...
# ---------------------------------------------------------------------------------
# TELA DE AUTENTICAÇÃO CORPORATIVA (DARK GLASSMORPHISM)
# ---------------------------------------------------------------------------------
def render_tela_autenticacao() -> None:
    """
    Renderiza a tela de autenticação oficial com estética Dark Glassmorphism,
    paleta Tailwind Dark, ambient radiance, validação de e-mail, senha e chave
    corporativa obrigatória (KEY-XXXX-XXXX-AUTH).
    """
    ...
    st.markdown("""
        ...
        <style>
        /* Esconder sidebar nativa e barras padrão na tela de login */
        ...
        .auth-top-header {
            ...
            border-bottom: 1px solid rgba(60, 74, 66, 0.35);
            display: flex;
            align-items: center;
        ...
        .auth-top-header .badge-ssl .pulse-dot {
            width: 7px;
            height: 7px;
            border-radius: 50%;
            background: #4cd7f6;
        ...
        .auth-container-wrapper {
            ...
            padding: 2.5rem 1rem;
            box-sizing: border-box;
            background: radial-gradient(circle at 50% 15%, rgba(78, 222, 163, 0.08) 0%, transparent 50%),
                        radial-gradient(circle at 80% 85%, rgba(76, 215, 246, 0.07) 0%, transparent 45%),
                        #0c1322;
        }
        ...
        .auth-glass-card {
            ...
            box-shadow: 0 20px 50px rgba(0, 0, 0, 0.5), 0 0 0 1px rgba(78, 222, 163, 0.1);
            position: relative;
            box-sizing: border-box;
        }
        ...
        /* Streamlit Input Customization for Auth */
        .auth-glass-card div[data-testid="stTextInput"] label,
        .auth-glass-card div[data-testid="stTextInput"] label p {
        ...
        /* Botão de Envio */
        .auth-glass-card div[data-testid="stFormSubmitButton"] button {
            width: 100% !important;
        ...
        .auth-glass-card div[data-testid="stFormSubmitButton"] button:hover {
        ...
        .auth-aux-links {
            margin-top: 20px;
            padding-top: 16px;
            border-top: 1px solid rgba(60, 74, 66, 0.4);
            display: flex;
            align-items: center;
            justify-content: space-between;
            font-size: 11.5px;
    ...
    st.markdown("""
        <div style="text-align: center;">
            ...
            <div class="auth-brand-title">Organizador de Contatos</div>
    ...
    with col_center:
        ...
        with st.form(key="form_autenticacao_corporativa", clear_on_submit=False):
            email_val = st.text_input(
                "E-mail Corporativo (Identificação SSO)",
        ...
        st.markdown("""
                <div class="auth-aux-links">
                    <span><span class="material-symbols-outlined" style="font-size: 13px;">key</span> Solicitar nova chave de acesso</span>
                    <span><span class="material-symbols-outlined" style="font-size: 13px;">headset_mic</span> Suporte Técnico TI</span>
        ...
        st.markdown("""
            <div class="telemetry-ribbon">
                <div class="telemetry-pill">
                    <span style="display:inline-block; width:6px; height:6px; border-radius:50%; background:#4edea3;"></span>
    ...
    # 3. Floating Companion Badge (Bottom Right)
    st.markdown(f"""
...
def run_app() -> None:
    ...
    # Gatekeeper: Se não autenticado, renderiza a tela de login exclusiva
    if not st.session_state.authenticated:
        render_tela_autenticacao()
        return

    # --- USUÁRIO AUTENTICADO: INTERFACE DO SISTEMA ---
    user_info = st.session_state.get("user_info") or {}
    nome_usuario = user_info.get("nome", "Colaborador Autorizado")
    ...
    with st.sidebar:
        st.markdown(f"""
            ...
            <div style="background: rgba(78, 222, 163, 0.1); border: 1px solid rgba(78, 222, 163, 0.25); border-radius: 8px; padding: 8px 12px; margin-top: 10px;">
                <div style="font-size: 11px; color: #4edea3; font-weight: 600;">{'👑 ADMINISTRADORA' if eh_admin else '👤 USUÁRIO AUTENTICADO'}</div>
                <div style="font-size: 12px; color: #dce2f7;">{nome_usuario}</div>
            </div>
        """, unsafe_allow_html=True)
        st.markdown("<hr class='glass-divider'>", unsafe_allow_html=True)

        # PAINEL DA ADMINISTRADORA (ALINE) - GERADOR DE CHAVES CORPORATIVAS
        if eh_admin:
            st.markdown('<div class="sidebar-section-title">🔑 GESTÃO DE CHAVES (ADMIN)</div>', unsafe_allow_html=True)
            with st.expander("⚡ Gerador de Chaves de Acesso", expanded=False):
                st.caption("Você é a responsável por gerar e gerenciar os tokens de segurança da empresa.")
                with st.form(key="form_gerar_chave_admin"):
                    novo_nome = st.text_input("Nome do Colaborador / Setor:", placeholder="ex: João Financeiro")
                    novo_email = st.text_input("E-mail Autorizado:", placeholder="ex: joao@empresa.com")
                    perfil_chave = st.selectbox("Perfil:", ["usuario", "admin"], index=0)
                    validade_dias = st.selectbox("Validade:", [30, 60, 90, None], format_func=lambda x: f"{x} dias" if x else "Sem Expiração (Permanente)")
                    
                    btn_gerar = st.form_submit_button("Gerar Nova Chave 🔐")
                    if btn_gerar:
                        chave_criada = criar_nova_chave(
                            responsavel=nome_usuario,
                            destinatario_nome=novo_nome,
                            destinatario_email=novo_email,
                            perfil=perfil_chave,
                            dias_validade=validade_dias
                        )
                        st.success("Chave gerada com sucesso!")
                        st.code(chave_criada["codigo"], language="text")
                        st.info(f"Atribuída a: {chave_criada['destinatario_nome']} ({chave_criada['destinatario_email']})")

            with st.expander("📋 Ver Chaves Cadastradas", expanded=False):
                chaves_listadas = listar_chaves()
                resumo = obter_resumo_estatistico()
                st.markdown(f"**Total:** {resumo['total']} | **Ativas:** {resumo['ativas']} | **Revogadas:** {resumo['revogadas']}")
                for c in chaves_listadas[:10]:
                    status_cor = "🟢" if c["status"] == "ativa" else "🔴"
                    col_k1, col_k2 = st.columns([3, 1])
                    with col_k1:
                        st.markdown(f"{status_cor} `{c['codigo']}`<br><small>{c['destinatario_nome']} • {c['status']}</small>", unsafe_allow_html=True)
                    with col_k2:
                        if c["status"] == "ativa" and c["codigo"] != "KEY-ALIN-9982-AUTH":
                            if st.button("Revogar", key=f"rev_{c['codigo']}"):
                                revogar_chave(c["codigo"])
                                st.rerun()
                        elif c["status"] == "revogada":
                            if st.button("Ativar", key=f"atv_{c['codigo']}"):
                                reativar_chave(c["codigo"])
                                st.rerun()
                st.markdown("<hr class='glass-divider'>", unsafe_allow_html=True)
        ...
        if st.button("🚪 Encerrar Sessão (Logout)", help="Efetua logoff seguro do sistema"):
            st.session_state.authenticated = False
            st.session_state.user_info = None
            st.session_state.is_admin = False
            st.rerun()
        st.markdown("""
            <div class="sidebar-footer">
                <div>Organizador de Contatos Enterprise</div>
            </div>
        """, unsafe_allow_html=True)
